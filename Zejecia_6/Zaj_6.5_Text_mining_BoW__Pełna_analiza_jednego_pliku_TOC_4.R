...
file_path <- "C:/Users/Julek/Desktop/Trump2025.txt"
...
# Wczytaj słowniki z plików csv ----
afinn <- read.csv("C:/Users/Julek/Desktop/afinn.csv", stringsAsFactors = FALSE)
bing <- read.csv("C:/Users/Julek/Desktop/bing.csv", stringsAsFactors = FALSE)
loughran <- read.csv("C:/Users/Julek/Desktop/loughran.csv", stringsAsFactors = FALSE)
nrc <- read.csv("C:/Users/Julek/Desktop/nrc.csv", stringsAsFactors = FALSE)

tidy_tokeny <- as_tibble(freq_df)

loughran_clean <- loughran %>%
  separate(word..sentiment., into = c("word", "sentiment"), sep = ",") %>%
...
# Użycie inner_join()
tidy_tokeny %>%
  inner_join(loughran_clean, relationship = "many-to-many")
...
sentiment_review_nrc <- tidy_tokeny %>%
  inner_join(nrc_clean, relationship = "many-to-many")
...
# dostosowanie słownika afinn 
afinn_clean <- afinn %>%
  separate(word..value., into = c("word", "value"), sep = ",") %>%
  mutate(value = gsub('"', '', value))


# Zliczanie sentymentu
sentiment_review_afinn <- tidy_tokeny %>%
  inner_join(afinn_clean)
